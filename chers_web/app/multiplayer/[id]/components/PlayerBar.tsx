...
function PlayerSlot({ name, color, isConnected, rightAligned }: PlayerSlotProps) {
  ...
  const colorIndicator = isWhite 
    ? "bg-white border-stone-300" 
    : "bg-stone-800 border-stone-600";
  
  return (
    <div className={`flex-1 flex items-center gap-3 p-3 rounded-lg ${rightAligned ? 'flex-row-reverse' : ''}`}>
      {/* Color Circle - just an empty circle, no dot inside */}
      <div className={`w-10 h-10 rounded-full border-2 ${colorIndicator} flex items-center justify-center flex-shrink-0`}>
        {/* Empty - no dot inside */}
      </div>
      ...
      <div className={`flex flex-col min-w-0 flex-1 ${rightAligned ? 'items-end text-right' : ''}`}>
        <div className={`flex items-center gap-2 ${rightAligned ? 'flex-row-reverse' : ''}`}>
          <span className="font-semibold truncate text-stone-900 dark:text-white">
            {name}
          </span>
        </div>
        <span className={`text-xs ${isConnected ? (isWhite ? 'text-stone-500' : 'text-stone-400') : 'text-yellow-600 font-semibold'}`}>
