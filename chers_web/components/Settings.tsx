...
export function SettingsTrigger(props: any) {
  ...
  return <button
    className={`h-4 w-4 rounded-full bg-black/50 hover:bg-black/60 dark:bg-white/30 dark:hover:bg-white/20 transition-colors border-2 border-black dark:border-white border-dashed ${className ?? ''}`}
    title="Open Settings"
    {...rest}
  ></button>
}
