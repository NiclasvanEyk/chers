...
export function Board({ state, dispatch }: BoardProps) {
  if (state.type === "ERROR") {
    return <span className="text-red-500 font-bold text-2xl">{state.error}</span>;
  }
    ...
    components.push(
      <div key="game-over" className="fixed inset-0 z-20">
        <div className="flex flex-col h-screen w-screen bg-black/75 place-content-center text-center">
          ...
          >
            <p className="text-3xl font-extrabold leading-tight">{state.winner} won</p>
            <button onClick={() => dispatch({ type: "BEGIN" })} className="text-white">
              Start Over
            </button>
          </dialog>
        </div>
