...
function multiplayerReducer(
  ...
  switch (command.type) {
    ...
    case "SELECT_TO": {
      ...
      const isPromotion = state.piece.figure === "Pawn" && (to.y === 0 || to.y === 7);
...
export function MultiplayerGame({
  ...
  const [localState, dispatch] = useReducer(multiplayerReducer, { type: "SELECTING_FROM", game });
      ...
      switch (action.type) {
        ...
        case "SELECT_TO":
          if (
            ...
            "x" in action.to &&
            localState.type === "SELECTING_TO"
          ) {
            const to = action.to as Coordinate;
            const isValidMove = localState.moves.some((m) => m.x === to.x && m.y === to.y);

            if (isValidMove) {
              // Check for promotion
              const isPromotion = localState.piece.figure === "Pawn" && (to.y === 0 || to.y === 7);
    ...
    <ChersSettingsProvider>
      <div className="min-h-screen p-4">
        ...
        <div className="max-w-2xl mx-auto">
          <PlayerBar myColor={myColor} myName={myName} opponent={opponent} isMyTurn={myTurn} />
