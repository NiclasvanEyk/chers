...
function handlePrivateEvent(state: MatchState, event: PrivateEvent): MatchState {
  ...
  switch (event.kind) {
    ...
    case "ColorsAssigned": {
      console.log(
        "🎨 ColorsAssigned event received! Player color:",
        event.player,
        "Current phase:",
        state.phase.kind,
      );
...
function handlePublicEvent(state: MatchState, event: PublicEvent): MatchState {
  ...
  switch (event.kind) {
    case "GameStarted": {
      console.log(
        "🎮 GameStarted received! Current phase:",
        state.phase.kind,
        "myColor:",
        state.myColor,
      );
...
export function useMatch(matchId: string) {
  ...
  const [state, dispatch] = useReducer(matchReducer, matchId, (id) => INITIAL_STATE(id, myName));
  ...
  const sendMove = useCallback(
    (from: Coordinate, to: Coordinate, promotion: PromotedFigure | null) => {
      ...
      const promotionPiece: PromotionPiece | null =
        promotion === null
          ? null
          : promotion === "Queen"
            ? "Q"
