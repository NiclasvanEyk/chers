  ...
  return (state: State, command: Command): State => {
    ...
    switch (command.type) {
      ...
      case "PROMOTE": {
        ...
        const result = adapter.nextState(game, state.from, state.to, command.to);
        if ("error" in result) {
          return { type: "ERROR", error: result.error, game };
        }
...
export function canMoveTo(state: State, to: Coordinate): boolean {
  ...
  return state.moves.find((move) => move.x === to.x && move.y === to.y) !== undefined;
}
...
export function useChers() {
  return useReducer(buildChersReducer(wasmAdapter), {} as unknown as any, initialChersState);
}
