import type { Dispatcher, State } from "@/lib/ui/state";
...
function arrowKeysNavigateCells(event: KeyboardEvent, cells: CellsByCoordinate) {
  const vector = {
    ArrowUp: [0, -1],
    ArrowDown: [0, 1],
    ArrowLeft: [-1, 0],
  ...
  const [x, y] = [activeCellCoordinates[0] + vector[0], activeCellCoordinates[1] + vector[1]];
